...
describe('delete confirmation', () => {
  test('uses the item name for a single deletion', () => {
    expect(deleteConfirmationTarget(['docs/report.txt'])).toBe('report.txt')
  })

  test('binds multi-item confirmation text to the selected item count', () => {
  ...
  test('requires an exact confirmation text', () => {
