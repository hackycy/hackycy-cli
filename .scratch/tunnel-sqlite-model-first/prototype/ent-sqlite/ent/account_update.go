...
func (_u *AccountUpdate) AddAllocationIDs(ids ...int) *AccountUpdate {
	_u.mutation.AddAllocationIDs(ids...)
	return _u
}

// AddAllocations adds the "allocations" edges to the Allocation entity.
func (_u *AccountUpdate) AddAllocations(v ...*Allocation) *AccountUpdate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_u *AccountUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.AllocationsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   account.AllocationsTable,
			Columns: []string{account.AllocationsColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(allocation.FieldID, field.TypeInt),
			},
		}
...
// AddAllocationIDs adds the "allocations" edge to the Allocation entity by IDs.
func (_u *AccountUpdateOne) AddAllocationIDs(ids ...int) *AccountUpdateOne {
	_u.mutation.AddAllocationIDs(ids...)
	return _u
}

// AddAllocations adds the "allocations" edges to the Allocation entity.
func (_u *AccountUpdateOne) AddAllocations(v ...*Allocation) *AccountUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
// ClearAllocations clears all "allocations" edges to the Allocation entity.
func (_u *AccountUpdateOne) ClearAllocations() *AccountUpdateOne {
	_u.mutation.ClearAllocations()
	return _u
}

// RemoveAllocationIDs removes the "allocations" edge to Allocation entities by IDs.
func (_u *AccountUpdateOne) RemoveAllocationIDs(ids ...int) *AccountUpdateOne {
	_u.mutation.RemoveAllocationIDs(ids...)
	return _u
}

// RemoveAllocations removes "allocations" edges to Allocation entities.
func (_u *AccountUpdateOne) RemoveAllocations(v ...*Allocation) *AccountUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
