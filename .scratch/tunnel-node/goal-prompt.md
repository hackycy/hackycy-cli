推进 `.scratch/tunnel-node/goal-runbook.md` 中启动本次 Goal 时唯一 `active` 的 Gate，直到该 Gate 通过或完成一次人工验收交接。开始时锁定该 Gate 的编号；该编号是本次 Goal 不可扩大的执行边界。只处理该 Gate，不得进入下一个 Gate。

启动时先读取 `.scratch/tunnel-node/goal-runbook.md` 的 Goal Ledger 和当前 Gate 的最后一条 Progress Log。若日志标记人工验收待确认且当前用户输入没有明确验收结果，只执行人工验收交接的终止规则，不读取代码、不运行验证、不重新实施。否则再阅读 `CLAUDE.md`、适用的 `AGENTS.md` / `CONTEXT.md`、`.scratch/tunnel-node/implementation-plan.md`、`.scratch/tunnel-node/goal-runbook.md`，以及当前 Gate 引用的 decision 文档和相关代码；以这些文件为唯一执行依据。
