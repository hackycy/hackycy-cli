...
describe('tunnel form schema', () => {
  it('maps an existing HTTP tunnel into editable defaults without exposing its password', () => {
  ...
  it('accepts the default HTTP form after a local port and domain are supplied', () => {
    expect(createTunnelSchema().safeParse(validValues()).success).toBe(true)
  })

  it('requires an HTTP domain and validates route locations', () => {
  ...
  it('keeps an existing Basic Auth password optional while requiring a new one', () => {
    const values = validValues()
  ...
  it('validates health timing and HTTP path only when health checks are enabled', () => {
    const values = validValues()
...
describe('tunnel form payload', () => {
  it('trims and filters dynamic headers and domains without changing the API shape', () => {
    const values = validValues()
  ...
  it('omits public HTTP fields and server ports for TCP/UDP payloads', () => {
    const values = validValues()
    values.protocol = 'tcp'
